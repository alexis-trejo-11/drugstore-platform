...
#
# Usage (from employee-service/docker/nginx/ssl/):
...
openssl req -x509 -nodes -days 365 \
    ...
    -keyout "$KEY" \
    -out    "$CRT" \
    -subj   "/C=MX/ST=Local/L=Local/O=DrugstorePlatform/OU=Dev/CN=employee-service" \
    -addext "subjectAltName=DNS:employee-service,DNS:localhost,IP:127.0.0.1"
