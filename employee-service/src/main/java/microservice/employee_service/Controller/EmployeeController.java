...
public class EmployeeController {
    ...
    public CompletableFuture<ResponseEntity<ResponseWrapper<List<EmployeeDTO>>>> getAllEmployees() {
        return employeeService.getAllEmployees().thenApply(employeeDTOS ->
                ResponseEntity.status(HttpStatus.OK).body(new ResponseWrapper<>(true, employeeDTOS, "Employees Successfully Fetched.", 200))
        );
    }
    ...
    public CompletableFuture<ResponseEntity<ResponseWrapper<EmployeeDTO>>> getEmployeeById(@PathVariable Long employeeId) {
        return employeeService.getEmployeeById(employeeId).thenApply(employeeDTO -> {
            if (employeeDTO.isEmpty()) {
                return ResponseEntity.status(HttpStatus.NOT_FOUND).body(new ResponseWrapper<>(true, null, "Employee With " + employeeId + " Not Found", 404));
            }
            return ResponseEntity.status(HttpStatus.OK).body(new ResponseWrapper<>(false, employeeDTO.get(), "Employee Successfully Fetched.", 200));
        });
    }
    ...
    public CompletableFuture<ResponseEntity<ResponseWrapper<?>>> updateEmployee(@RequestBody @Valid EmployeeUpdateDTO employeeUpdateDTO, BindingResult bindingResult) {
        ...
        return employeeService.updateEmployee(employeeUpdateDTO).thenApply(updateEmployeeResult -> {
            ...
            return ResponseEntity.status(HttpStatus.OK).body(new ResponseWrapper<>(true, null, "Employee Successfully Updated.", 200));
        });
    }
    ...
    public CompletableFuture<ResponseEntity<ResponseWrapper<Void>>> deleteEmployee(@PathVariable Long employeeId) {
        return employeeService.getEmployeeById(employeeId).thenCompose(employeeDTO -> {
            if (employeeDTO.isEmpty()) {
                return CompletableFuture.completedFuture(ResponseEntity.status(HttpStatus.NOT_FOUND).body(new ResponseWrapper<>(false, null, "Employee With " + employeeId + " Not Found", 404)));
            }
            return employeeService.deleteEmployee(employeeId).thenApply(voidResult ->
                    ResponseEntity.status(HttpStatus.OK).body(new ResponseWrapper<>(true, null, "Employee Successfully Deleted.", 200))
            );
        });
    }
}
