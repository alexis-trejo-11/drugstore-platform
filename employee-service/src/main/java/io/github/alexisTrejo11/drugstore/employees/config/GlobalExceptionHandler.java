...
public class GlobalExceptionHandler {
  ...
  public ResponseEntity<ResponseWrapper<?>> handleHandlerMethodValidationException(
    ...
    log.warn("Method validation failed: {} validation error(s)", ex.getAllErrors().size());
    ...
    List<MessageSourceResolvable> crossErrors = ex.getCrossParameterValidationResults();
    for (int i = 0; i < crossErrors.size(); i++) {
      var cp = crossErrors.get(i);
      String errorMessage =
          cp.getDefaultMessage() != null ? cp.getDefaultMessage() : "Validation failed";
      validationErrors.merge("crossParameter[" + i + "]", errorMessage,
          (existing, newMsg) -> existing + "; " + newMsg);
    }
