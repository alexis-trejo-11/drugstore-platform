...
All containerization for **order-service** lives under this directory: the image definition, reverse proxy, observability configs, and Compose files.

Environment variables live in a **single `.env` file at the project root** (`order-service/.env`). Copy from `.env.example` and run Compose from the root.
...
cd order-service
cp .env.example .env
# Edit .env — set JWT_SECRET_KEY, GITHUB_TOKEN, and DB/Redis URLs for your profile
...
|------|----------------|-------------|
| `docker-compose.full.yml` | Nginx, order-service, PostgreSQL, Redis, Prometheus, Loki, Grafana | Local development with everything in Docker |
| `docker-compose.app.yml` | Nginx + order-service only | Deploy the app against external DB/Redis (host or cloud) |
...
`docker-compose.full.yml` only asserts **inter-container communication** defaults on `order-service` (e.g. `DATASOURCE_URL` pointing at `postgres`, `REDIS_URL` pointing at `redis`) so the bundled local stack works out of the box. Override those in `.env` for production.
...
- **`prod`** — starts only Nginx and order-service; set `DATASOURCE_URL` and `REDIS_URL` to your cloud endpoints in `.env`.
...
From **`order-service/`** (project root):
...
All commands assume your current directory is **`order-service/`**.
...
```bash
docker compose -f docker/docker-compose.full.yml --env-file .env up -d --scale order-service=3
```
...
DATASOURCE_URL=jdbc:postgresql://host.docker.internal:5432/order_db
...
|----------|--------------|------|
| `DATASOURCE_URL` | `jdbc:postgresql://postgres:5432/order_db` | Cloud Postgres JDBC URL |
...
```bash
# View logs
docker compose -f docker/docker-compose.full.yml --env-file .env logs -f order-service
...
docker compose -f docker/docker-compose.full.yml --env-file .env up -d --build order-service
```
...
**App cannot reach Postgres in app-only local mode** — Ensure Postgres listens on the host (`5432`) and set `DATASOURCE_URL=jdbc:postgresql://host.docker.internal:5432/order_db` in `.env`.
