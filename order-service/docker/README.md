...
- **`--profile prod`** — starts only Nginx and order-service; set `POSTGRES_HOST`, `REDIS_HOST`, etc. to your cloud endpoints in `.env.prod`.
...
From the **`order-service/docker/`** directory:
...
All commands below assume your current directory is **`order-service/docker/`**.
...
docker compose \
  ...
  --env-file .env \
  --env-file .env.local \
  up -d --scale order-service=3
```

### Full stack — prod (app containers + cloud infrastructure)

Starts only Nginx and order-service. Database, cache, and monitoring are **not** started; configure cloud endpoints in `.env.prod`.
...
```bash
# View logs
docker compose -f docker-compose.full.yml --profile local logs -f order-service
...
docker compose -f docker-compose.full.yml --profile local up -d --build order-service
```
