package io.github.alexisTrejo11.drugstore.order.config.ratelimit;
...
public class RateLimitAspect {
  ...
  public Object checkRateLimit(ProceedingJoinPoint joinPoint, RateLimit rateLimit) throws Throwable {
    ...
    String yamlProfileKey = yamlProfileKey(profile);
    RateLimitProperties.ProfileConfig config = properties.getProfiles()
        .getOrDefault(yamlProfileKey, getDefaultConfig());
  ...
  private String buildRateLimitKey(RateLimit rateLimit, ProceedingJoinPoint joinPoint) {
    ...
    return String.format("profile:%s:%s:%s:%s",
        yamlProfileKey(rateLimit.profile()),
        rateLimit.type().name().toLowerCase(Locale.ROOT),
        subjectIdentifier,
        actionIdentifier);
  }

  private static String yamlProfileKey(RateLimitProfile profile) {
    return profile.name().toLowerCase(Locale.ROOT).replace('_', '-');
  }
