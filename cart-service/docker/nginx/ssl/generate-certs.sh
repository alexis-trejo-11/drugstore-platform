...
#
# Usage (from cart-service/docker/nginx/ssl/):
...
if [ -d "$KEY" ] || [ -d "$CRT" ]; then
    ...
    echo "  cd <cart-service> && docker compose -f docker/docker-compose.full.yml --env-file .env stop nginx"
    echo "  rm -rf \"$KEY\" \"$CRT\""
    exit 1
fi
...
openssl req -x509 -nodes -days 365 \
    ...
    -keyout "$KEY" \
    -out    "$CRT" \
    -subj   "/C=MX/ST=Local/L=Local/O=DrugstorePlatform/OU=Dev/CN=cart-service" \
    -addext "subjectAltName=DNS:cart-service,DNS:localhost,IP:127.0.0.1"
