...
All containerization for **cart-service** lives under this directory: the image definition, reverse proxy, observability configs, and Compose files.

Environment variables live in a **single `.env` file at the project root** (`cart-service/.env`). Copy from `.env.example` and run Compose from the root.
...
|------|----------------|-------------|
| `docker-compose.full.yml` | Nginx, cart-service, PostgreSQL, Redis, Prometheus, Loki, Grafana | Local development with everything in Docker |
| `docker-compose.app.yml` | Nginx + cart-service only | Deploy the app against external DB/Redis (host or cloud) |
...
`docker-compose.full.yml` only asserts **inter-container communication** defaults on `cart-service` (e.g. `DATASOURCE_URL` pointing at `postgres`, `REDIS_URL` pointing at `redis`) so the bundled local stack works out of the box. Override those in `.env` for production.
...
- **`prod`** — starts only Nginx and cart-service; set `DATASOURCE_URL` and `REDIS_URL` to your cloud endpoints in `.env`.
...
From **`cart-service/`** (project root):
...
All commands assume your current directory is **`cart-service/`**.
...
DATASOURCE_URL=jdbc:postgresql://host.docker.internal:5432/cart_db
...
|---------|-------------------------|
| API (HTTPS via Nginx) | `https://localhost/api/v2/carts` |
...
|----------|--------------|------|
| `DATASOURCE_URL` | `jdbc:postgresql://postgres:5432/cart_db` | Cloud Postgres JDBC URL |
| `REDIS_URL` | `redis://redis:6379` | Cloud Redis URL |
| `KAFKA_BOOTSTRAP_SERVERS` | `localhost:9092` (host) or broker DNS in `.env` | Cloud Kafka brokers |
...
```bash
# View logs
docker compose -f docker/docker-compose.full.yml --env-file .env logs -f cart-service
...
docker compose -f docker/docker-compose.full.yml --env-file .env up -d --build cart-service
```
...
**App cannot reach Postgres in app-only local mode** — Ensure Postgres listens on the host (`5432` or `5433`) and set `DATASOURCE_URL=jdbc:postgresql://host.docker.internal:5432/cart_db` in `.env`.

**Profile prod still tries to connect to `postgres`** — Set `COMPOSE_PROFILES=prod` and cloud `DATASOURCE_URL` / `REDIS_URL` in `.env`.

**Kafka consumer not connecting** — Set `KAFKA_BOOTSTRAP_SERVERS` in `.env` to reachable broker addresses for your deployment.
