...
All containerization for **product-service** lives under this directory: the image definition, reverse proxy, observability configs, and Compose files.

Environment variables live in a **single `.env` file at the project root** (`product-service/.env`). Copy from `.env.example` and run Compose from the root.
...
cd product-service
cp .env.example .env
# Edit .env — set JWT_SECRET_KEY, GITHUB_TOKEN, and connection URLs for your profile
chmod +x docker/nginx/ssl/generate-certs.sh
./docker/nginx/ssl/generate-certs.sh   # local/dev only

# Full local stack also needs the shared Kafka cluster:
...
|------|----------------|-------------|
| `docker-compose.full.yml` | Nginx, product-service, PostgreSQL, Redis, Prometheus, Loki, Grafana | Local development with everything in Docker |
| `docker-compose.app.yml` | Nginx + product-service only | Deploy the app against external DB/Redis/Kafka (host or cloud) |
...
`docker-compose.full.yml` only asserts **inter-container communication** defaults on `product-service` (e.g. `DATASOURCE_URL` pointing at `postgres`, `REDIS_URL` pointing at `redis`) so the bundled local stack works out of the box. Override those in `.env` for production.
...
- **`prod`** — starts only Nginx and product-service; set `DATASOURCE_URL`, `REDIS_URL`, and `SPRING_KAFKA_BOOTSTRAP_SERVERS` to your cloud endpoints in `.env`.
...
From **`product-service/`** (project root):
...
# Full local stack only — start Kafka (creates drugstore-kafka-network)
cd ../infrastructure/kafka && docker compose up -d && cd -
```

## How to run

All commands assume your current directory is **`product-service/`**.
...
```bash
docker compose -f docker/docker-compose.full.yml --env-file .env up -d --scale product-service=3
```
...
> If Compose fails because `drugstore-kafka-network` does not exist, create it once with `docker network create drugstore-kafka-network`, or use `docker-compose.app.yml` with cloud Kafka endpoints.
...
DATASOURCE_URL=jdbc:postgresql://host.docker.internal:5432/product_db
...
| Swagger UI (direct, local dev) | `http://localhost:8086/swagger-ui.html` |
| Prometheus UI | `http://localhost:9090` |
| Loki | `http://localhost:3100/ready` |
| Grafana | `http://localhost:3000` (admin / admin by default) |

External API traffic goes through Nginx on ports **80** and **443**. Port **8086** exposes the app directly on the host for dev tools (Swagger, etc.) without going through Nginx.
...
|----------|--------------|------|
| `DATASOURCE_URL` | `jdbc:postgresql://postgres:5432/product_db` | Cloud Postgres JDBC URL |
...
```bash
# View logs
docker compose -f docker/docker-compose.full.yml --env-file .env logs -f product-service
...
docker compose -f docker/docker-compose.full.yml --env-file .env up -d --build product-service
```
...
**App cannot reach Postgres in app-only local mode** — Ensure Postgres listens on the host (`5432`) and set `DATASOURCE_URL=jdbc:postgresql://host.docker.internal:5432/product_db` in `.env`.
