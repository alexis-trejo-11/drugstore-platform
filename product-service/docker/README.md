...
- **`--profile prod`** — starts only Nginx and product-service; set `POSTGRES_HOST`, `REDIS_HOST`, `SPRING_KAFKA_BOOTSTRAP_SERVERS`, etc. to your cloud endpoints in `.env.prod`.
...
From the **`product-service/docker/`** directory:
...
All commands below assume your current directory is **`product-service/docker/`**.
...
Starts only Nginx and product-service. Database, cache, Kafka, and monitoring are **not** started; configure cloud endpoints in `.env.prod`.
...
```bash
# View logs
docker compose -f docker-compose.full.yml --profile local logs -f product-service
...
docker compose -f docker-compose.full.yml --profile local up -d --build product-service
```
...
**Profile prod still tries to connect to `postgres`** — Use `--env-file .env.prod` so `POSTGRES_HOST`, `REDIS_HOST`, and `SPRING_KAFKA_BOOTSTRAP_SERVERS` point to cloud hosts, not Docker service names.
