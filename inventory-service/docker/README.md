...
Environment variables live in a **single `.env` file at the project root** (`inventory-service/.env`). Copy from `.env.example` and run Compose from the root.
...
`docker-compose.full.yml` only asserts **inter-container communication** defaults on `inventory-service` (e.g. `DATASOURCE_URL` pointing at `postgres`, `REDIS_URL` pointing at `redis`) so the bundled local stack works out of the box. Override those in `.env` for production.
...
- **`prod`** — starts only Nginx and inventory-service; set `DATASOURCE_URL` and `REDIS_URL` to your cloud endpoints in `.env`.
...
From **`inventory-service/`** (project root):
...
All commands assume your current directory is **`inventory-service/`**.
...
```bash
docker compose -f docker/docker-compose.full.yml --env-file .env up -d --scale inventory-service=3
```
...
DATASOURCE_URL=jdbc:postgresql://host.docker.internal:5432/inventory_db
...
|----------|--------------|------|
| `DATASOURCE_URL` | `jdbc:postgresql://postgres:5432/inventory_db` | Cloud Postgres JDBC URL |
...
```bash
# View logs
docker compose -f docker/docker-compose.full.yml --env-file .env logs -f inventory-service
...
docker compose -f docker/docker-compose.full.yml --env-file .env up -d --build inventory-service
```
...
**App cannot reach Postgres in app-only local mode** — Ensure Postgres listens on the host (`5432`) and set `DATASOURCE_URL=jdbc:postgresql://host.docker.internal:5432/inventory_db` in `.env`.
