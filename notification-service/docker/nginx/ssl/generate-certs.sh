...
#
# Usage (from notification-service/docker/nginx/ssl/):
...
if [ -d "$KEY" ] || [ -d "$CRT" ]; then
    ...
    echo "  cd notification-service/docker && docker compose -f docker-compose.full.yml --profile local stop nginx"
    echo "  rm -rf \"$KEY\" \"$CRT\""
    exit 1
fi
...
openssl req -x509 -nodes -days 365 \
    ...
    -keyout "$KEY" \
    -out    "$CRT" \
    -subj   "/C=MX/ST=Local/L=Local/O=DrugstorePlatform/OU=Dev/CN=notification-service" \
    -addext "subjectAltName=DNS:notification-service,DNS:localhost,IP:127.0.0.1"
