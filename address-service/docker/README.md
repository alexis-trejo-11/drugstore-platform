...
Environment variables live in a **single `.env` file at the project root** (`address-service/.env`). Copy from `.env.example` and run Compose from the root.
...
cd address-service
cp .env.example .env
# Edit .env — set JWT_SECRET_KEY, GITHUB_TOKEN, and DB/Redis hosts for your profile
...
`docker-compose.full.yml` only asserts **inter-container communication** defaults on `address-service` (e.g. `DATASOURCE_URL` pointing at `postgres`, `REDIS_URL` pointing at `redis`) so the bundled local stack works out of the box. Override those in `.env` for production.
...
- **`prod`** — starts only Nginx and address-service; set `DATASOURCE_URL` and `REDIS_URL` to your cloud endpoints in `.env`.
...
From **`address-service/`** (project root):
...
All commands assume your current directory is **`address-service/`**.
...
```bash
docker compose -f docker/docker-compose.full.yml --env-file .env up -d --scale address-service=3
```
...
DATASOURCE_URL=jdbc:postgresql://host.docker.internal:5432/address_db
...
|----------|--------------|------|
| `DATASOURCE_URL` | `jdbc:postgresql://postgres:5432/address_db` | Cloud Postgres JDBC URL |
...
```bash
# View logs
docker compose -f docker/docker-compose.full.yml --env-file .env logs -f address-service
...
docker compose -f docker/docker-compose.full.yml --env-file .env up -d --build address-service
```
...
**App cannot reach Postgres in app-only local mode** — Ensure Postgres listens on the host (`5432`) and set `DATASOURCE_URL=jdbc:postgresql://host.docker.internal:5432/address_db` in `.env`.
