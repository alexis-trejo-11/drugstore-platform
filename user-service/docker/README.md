...
All containerization for **user-service** lives under this directory: the image definition, reverse proxy, observability configs, and Compose files.

Environment variables live in a **single `.env` file at the project root** (`user-service/.env`). Copy from `.env.example` and run Compose from the root.
...
|------|----------------|-------------|
| `docker-compose.full.yml` | Nginx, user-service, PostgreSQL, Redis, Prometheus, Loki, Grafana | Local development with everything in Docker |
| `docker-compose.app.yml` | Nginx + user-service only | Deploy the app against external DB/Redis/Kafka (host or cloud) |
...
`docker-compose.full.yml` only asserts **inter-container communication** defaults on `user-service` (e.g. `DATASOURCE_URL` pointing at `postgres`, `REDIS_URL` pointing at `redis`) so the bundled local stack works out of the box. Override those in `.env` for production.
...
- **`prod`** — starts only Nginx and user-service; set `DATASOURCE_URL`, `REDIS_URL`, and `SPRING_KAFKA_BOOTSTRAP_SERVERS` to your cloud endpoints in `.env`.
...
From **`user-service/`** (project root):
...
All commands assume your current directory is **`user-service/`**.
...
```bash
docker compose -f docker/docker-compose.full.yml --env-file .env up -d --scale user-service=3
```

### Full stack — prod (app containers + cloud infrastructure)

Set `COMPOSE_PROFILES=prod` and cloud `DATASOURCE_URL` / `REDIS_URL` / `SPRING_KAFKA_BOOTSTRAP_SERVERS` in `.env`, then:
...
DATASOURCE_URL=jdbc:postgresql://host.docker.internal:5432/user_db
...
Host port mappings for bundled infra (from `.env`): Postgres **5431**, Redis **6378**.
...
|----------|--------------|------|
| `DATASOURCE_URL` | `jdbc:postgresql://postgres:5432/user_db` | Cloud Postgres JDBC URL |
...
```bash
# View logs
docker compose -f docker/docker-compose.full.yml --env-file .env logs -f user-service
...
docker compose -f docker/docker-compose.full.yml --env-file .env up -d --build user-service
```
...
**App cannot reach Postgres in app-only local mode** — Ensure Postgres listens on the host (`5432`) and set `DATASOURCE_URL=jdbc:postgresql://host.docker.internal:5432/user_db` in `.env`.
