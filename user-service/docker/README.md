...
- **`--profile prod`** — starts only Nginx and user-service; set `POSTGRES_HOST`, `REDIS_HOST`, `SPRING_KAFKA_BOOTSTRAP_SERVERS`, etc. to your cloud endpoints in `.env.prod`.
...
From the **`user-service/docker/`** directory:
...
All commands below assume your current directory is **`user-service/docker/`**.
...
Starts only Nginx and user-service. Database, cache, Kafka, and monitoring are **not** started; configure cloud endpoints in `.env.prod`.
...
```bash
# View logs
docker compose -f docker-compose.full.yml --profile local logs -f user-service
...
docker compose -f docker-compose.full.yml --profile local up -d --build user-service
```
