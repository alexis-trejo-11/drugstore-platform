...
#
# Usage (from auth-service/docker/nginx/ssl/):
...
if [ -d "$KEY" ] || [ -d "$CRT" ]; then
    ...
    echo "  cd auth-service/docker && docker compose -f docker-compose.full.yml --profile local stop nginx"
    echo "  rm -rf \"$KEY\" \"$CRT\""
    exit 1
fi
...
openssl req -x509 -nodes -days 365 \
    ...
    -keyout "$KEY" \
    -out    "$CRT" \
    -subj   "/C=MX/ST=Local/L=Local/O=DrugstorePlatform/OU=Dev/CN=auth-service" \
    -addext "subjectAltName=DNS:auth-service,DNS:localhost,IP:127.0.0.1"
...
echo "  Cert: $CRT"
echo ""
echo "These files are mounted into the nginx container by docker/docker-compose.*.yml."
